...
void inserirElemento(Lista* li,void* dado){
...
void inserirElementoFinal(Lista* li,void* dado){
	...
	else{//caso seja o primeiro elemento a ser criado, chama a funcao inserirElemento normalmente
		inserirElemento(li,dado);
	}
}

void inserirElementoOrdenado(Lista* li,void* dado,void* funcDeOrdenacao){
	ordenar *ordenador = funcDeOrdenacao;//recebe o ponteiro para função passado como parâmetro;
	int encontrou = 0;
	if(*li!=NULL){//testa se é o primeiro elemento a ser criado
		elem *temp = *li;
		while(!encontrou){
			if(ordenador(dado,(*li)->dado)){
				encontrou = 1;
			}else{
				*li=(*li)->prox;
			}
		...
		elemento *e1 = (elem*)malloc(sizeof(elem));// aloca um novo elemento
		e1->dado = dado;//atribui o dado desejado que foi passado por parametro
		e1->prox = (*li)->prox;// atribui o endereço do próximo elemento
		(*li)->prox = e1;
		*li = temp;// recupera o endereco do primeiro elemento da lista
	}
	else{//caso seja o primeiro elemento a ser criado, chama a funcao inserirElemento normalmente
